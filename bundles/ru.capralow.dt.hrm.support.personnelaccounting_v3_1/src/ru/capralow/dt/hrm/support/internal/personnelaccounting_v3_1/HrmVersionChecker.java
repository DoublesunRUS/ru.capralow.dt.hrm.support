...
public final class HrmVersionChecker
{

    public static boolean checkHrmVersion(IV8Project v8Project, String startVersion)
    {
        String sslVersion = getHrmVersion(v8Project);

        return compareVersions(startVersion, sslVersion) == 0;
    }

    public static boolean checkHrmVersion(IV8Project v8Project, String startVersion, String endVersion)
    {
        String sslVersion = getHrmVersion(v8Project);
    ...
    private static String getHrmVersion(IV8Project v8Project)
    {
        String version = ""; //$NON-NLS-1$

        MdObject mdObject = MdUtils.getMdObject(
            MessageFormat.format(MdUtils.MD_OBJECT, "ОбщийМодуль", "ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый"), //$NON-NLS-1$ //$NON-NLS-2$
            v8Project);
        if (mdObject == null)
            return version;
